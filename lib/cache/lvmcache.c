				   ...
				   const char *vgname, const char *vgid, uint32_t vgstatus)
{
	...
	if (!created) {
		if (info->dev != dev) {
			...
			/*
			 * SUSE: we warm the user to disable lvmetad manually because it's not easy
			 * to backport all the "auto-disable-lvmetad" patches.
			 */
			if (!_found_duplicate_pvs && lvmetad_used()) {
				log_warn("WARNING: Please disable lvmetad which does not support duplicate PVs.");
